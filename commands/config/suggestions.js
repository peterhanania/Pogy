...
let fail = message.client.emoji.fail
      ...
      if (args.includes('disable')) {
        ...
        if(guildDB.suggestion.suggestionChannelID === null) return message.channel.send ({ embeds: [new MessageEmbed()
        .setColor(message.guild.me.displayHexColor)
    ...
    if(guildDB.suggestion.suggestionChannelID === channel.id) return message.channel.send ({ embeds: [new MessageEmbed()
          .setColor(message.guild.me.displayHexColor)
...
if(guildDB.suggestion.decline == "false"){
  return message.channel.send ({ embeds: [new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} Staff can't approve or decline Suggestions in this guild.`).setFooter({text: 'https://pogy.xyz/'}).setTimestamp().setColor('RED')]});
}
...
if(!args[1]) return message.channel.send ({ embeds: [new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting12}`).setFooter({text: 'https://pogy.xyz/'}).setTimestamp().setColor('RED')]});
...
var suggestionMsg = await channel.messages.fetch(args[1])
             ...
             message.channel.send ({ embeds: [new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting13}`).setFooter({text: 'https://pogy.xyz/'}).setTimestamp().setColor('RED')]});
            return;
}
...
let description = suggestionMsg.embeds[0].description;
 ...
 if(suggestionMsg.embeds[0].title !== `${language.suggesting3}`) {
   ...
   if(suggestionMsg.embeds[0].title === `${language.suggesting14}`){
             message.channel.send ({ embeds: [new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting15}`).setFooter({text: 'https://pogy.xyz/'}).setTimestamp().setColor('RED')]});
   } else {
             message.channel.send ({ embeds: [new MessageEmbed().setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ format: 'png' })).setDescription(`${fail} ${language.suggesting16}`).setFooter({text: 'https://pogy.xyz/'}).setTimestamp().setColor('RED')]});
   }
